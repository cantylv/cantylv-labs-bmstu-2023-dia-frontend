...
import { Service, mock_services } from '../mockData';
  ...
  const ButtonHandler = () => {
    ...
    fetch(url)
      ...
      .catch((error) => {
        ...
        if (startDate) {
          filteredServices = filteredServices.filter(
            (service) => new Date(service.date_start) >= new Date(startDate)
          );
        }
        if (endDate) {
          filteredServices = filteredServices.filter(
            (service) => new Date(service.date_start) <= new Date(endDate)
          );
        }
        if (salaryStart) {
          filteredServices = filteredServices.filter(
        ...
        console.error(
          'Ошибка при запросе отфильтрованного списка услуг:',
          error
        );
      });
  };
      ...
      <div className="filterMenu">
        ...
        <div className="filterSalary">
          ...
          <input
            ...
            onChange={(event) => {
              setSalaryStart(parseInt(event.target.value));
            }}
          />
          ...
          <input
            className="salaryBlock"
            onChange={(event) => {
              setSalaryEnd(parseInt(event.target.value));
            }}
          />
          <Button type="button" onClick={ButtonHandler}>
            Найти
